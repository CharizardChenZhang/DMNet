...
class ScanData:
    ...
    def load_full_scan(self, file_path, cfg):
        ...
        l['output_facet_vertex_idx'] = np.fromfile(file_names['output_facet_vertex_idx'], dtype=np.int32,
                                                   sep=' ').reshape(-1, 3)
        l['output_facet_nei_cell'] = np.fromfile(file_names['output_facet_nei_cell'], dtype=np.int32, sep=' ').reshape(
            -1, 2)
...
def create_full_data(scan, cfg):
    ...
    facet_feature, length_max = get_facet_feature(out.facet_vertex_idx, current_xyz,
                                                  out.facet_nei_cell, infinite_cell, cell_ball_centers)
    out.batch_facet_feature = facet_feature
    length_max = length_max.squeeze(1)
    out.facet_length_max = length_max / torch.max(length_max, dim=0)[0]
...
def get_cell_feature(cell_vertex_xzy):
    ...
    cell_ball_radius = torch.clamp(cell_ball_radius, max=20000.)
    cell_ball_radius = cell_ball_radius / 1000.
    ...
    cell_feature = cell_feature / torch.max(cell_feature, dim=0)[0]
...
def get_facet_feature(facet_vertex_idx, current_xyz, facet_nei_cell, infinite_cell, cell_ball_centers):
    ...
    fn = f.normalize(torch.cross(ab, -1.0 * ca, dim=1), p=2, dim=-1)
    facet_area = torch.norm(torch.cross(ab, -ca, dim=1), dim=1, keepdim=True) / 2.0
    ...
    facet_feature = facet_feature / torch.max(facet_feature, dim=0)[0]
