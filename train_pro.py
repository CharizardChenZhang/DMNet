...
exp_lr_scheduler = lr_scheduler.MultiStepLR(optimizer, [5,10], gamma=0.1)
train_data = DTU.DTUDelDataset(cfg, "train")
train_data_loader = dataloader.DataListLoader(train_data, cfg["batch_size"], shuffle=True,
                                                     num_workers=cfg["num_workers"])
val_data = DTU.DTUDelDataset(cfg, "val")
val_data_loader = dataloader.DataListLoader(val_data, cfg["batch_size"], num_workers=cfg["num_workers"])
...
for epoch in range(init_epoch, cfg['epochs']):
    ...
    if epoch % 1 == 0:
        ...
        print("Saving extra model", cfg["weight_ratio"])
        torch.save(train_model.state_dict(), extra_model_path)
        loss_array = np.asarray([tmp_loss, tmp_loss1, tmp_loss2, tmp_loss3])
